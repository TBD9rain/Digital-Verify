...
class InputMon #(
    ...
    task catch;
        ...
        bit [DATA_WIDTH - 1: 0] data_in_a0;
        bit [DATA_WIDTH - 1: 0] data_in_a1;
        bit [DATA_WIDTH - 1: 0] data_in_b0;
        bit [DATA_WIDTH - 1: 0] data_in_b1;

        longint unsigned timestamp;

        if (vif.mon_cb.data_in_vld) begin
            data_in_a0 = vif.mon_cb.data_in_a0;
            data_in_a1 = vif.mon_cb.data_in_a1;
            data_in_b0 = vif.mon_cb.data_in_b0;
            data_in_b1 = vif.mon_cb.data_in_b1;

            timestamp = vif.mon_cb.clk_cnt;

            txn_caught = new(data_in_a0, data_in_a1, data_in_b0, data_in_b1, timestamp);
